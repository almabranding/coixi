...
{*
** Retro compatibility for PrestaShop version 
< 1.4.2.5 with a recent theme
...
{else}
{l s='Choose a delivery address:'}{/if}</label>
...
{if $address.id_address == $cart->id_address_delivery}selected="selected"{/if}>{$address.alias|escape:'htmlall':'UTF-8'}</option>
...
<input type="checkbox" name="same" id="addressesAreEquals" value="1" onclick="updateAddressesDisplay();{if $opc}updateAddressSelection();{/if}" 
