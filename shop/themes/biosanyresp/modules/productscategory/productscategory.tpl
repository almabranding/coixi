...
SA <contact@prestashop.com>*  @copyright  2007-2012 PrestaShop SA*  @version  Release: $Revision: 8337 $*  @license    http://opensource.org/licenses/afl-3.0.php  Academic Free 
...
<ul {if count($categoryProducts) > 5}style="width: {math equation="width * nbImages" width=340 nbImages=$categoryProducts|@count}px"{/if}>			
{foreach from=$categoryProducts item='categoryProduct' name=categoryProduct}		

<li>				
<a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" class="lnk_img" title="{$categoryProduct.name|htmlspecialchars}"><img src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image, 'list')}" alt="{$categoryProduct.name|htmlspecialchars}" /></a>	
...
<a class="lnk_more" href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" title="{l s='View' mod='productscategory'}"></a>				
...
$('#productscategory_list').trigger('goto', [{$middlePosition}-3]);	</script>
