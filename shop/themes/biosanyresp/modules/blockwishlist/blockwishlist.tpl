...
SA <contact@prestashop.com>*  @copyright  2007-2012 PrestaShop SA*  @version  Release: $Revision: 6844 $*  @license    http://opensource.org/licenses/afl-3.0.php  Academic Free 
...
<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category_rewrite)}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|truncate:15:'...'|escape:'htmlall':'UTF-8'}</a>					
<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product}', {$product.id_product_attribute}, '0', '{$token}');" title="{l s='remove this product from my wishlist' mod='blockwishlist'}">
<img src="{$img_dir}green_icons/delete-green-16.png" width="16" height="16" alt="{l s='Delete'}" class="icon" onmouseover="this.src='{$img_dir}green_icons/delete-black-16.png';" onmouseout="this.src='{$img_dir}green_icons/delete-green-16.png';" />
</a>				
</dt>				
...
<option value="{$wishlist.id_wishlist}"{if $id_wishlist eq $wishlist.id_wishlist or ($id_wishlist == false and $smarty.foreach.i.first)} selected="selected"{/if}>{$wishlist.name|truncate:15:'...'|escape:'htmlall':'UTF-8'}</option>			
{/foreach}			
</select>		
{/if}			
