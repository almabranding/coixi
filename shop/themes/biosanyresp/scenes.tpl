...
<div class="screen_scene" id="screen_scene_{$scene->id}" style="background:transparent url({$base_dir}img/scenes/{$scene->id}-large_scene.jpg); height:{$largeSceneImageType.height}px; width:{$largeSceneImageType.width}px; 
...
{if isset($product.details->new) AND $product.details->new}<span class="new">{l s='New'}</span>
{/if}
