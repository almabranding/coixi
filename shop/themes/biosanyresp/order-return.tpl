...
{assign var='quantityDisplayed' value=0}				{foreach from=$returnedCustomizations item='customization' name=products}					
...
</tr>						{assign var='productId' value=$customization.product_id}						
...
{/if}				{/foreach}				{if $product.product_quantity > $quantityDisplayed}					
<tr class="{if $smarty.foreach.products.first}first_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if}">						
...
<br />
- {l s='Please print out the'} <a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$orderRet->id|intval}")}">{l s='PDF return slip'}</a> {l s='and include it with your package.'}		
<br />
- {l s='Please see PDF return slip'} (<a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$orderRet->id|intval}")}">{l s='for the correct address'}</a>)		
<br /><br />		
...
<br /><br />
<a href="{$link->getPageLink('contact', true)}">{l s='Please let us know if you have any questions.'}</a>		
<br />		
